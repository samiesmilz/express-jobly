...
describe("findMatch", () => {
  const newCompany = {
    ...
    numEmployees: 3,
    logoUrl: "http://real.img",
  };

  test("filter by name", async () => {
    let company = await Company.create(newCompany);
    let searchResults = await Company.findMatch({ name: "Real LLC" });
    let wideSearch = await Company.findMatch({ name: "C" });
    expect(searchResults[0]).toEqual(company);
    expect(wideSearch.length).toBe(4);
  });

  test("filter by minEmployees", async () => {
    let searchResults = await Company.findMatch({ name: "C", minEmployees: 2 });
  ...
  test("filter by maxEmployess", async () => {
    let searchResults = await Company.findMatch({ maxEmployees: 3 });
    expect(searchResults.length).toBe(3);
  });

  test("filter by combinations", async () => {
    let searchResults = await Company.findMatch({
      name: "C",
      minEmployees: 2,
      maxEmployees: 3,
    });
    let searchResultsWithoutName = await Company.findMatch({
      minEmployees: 1,
      maxEmployees: 3,
    });
    expect(searchResults.length).toBe(2);
    expect(searchResultsWithoutName.length).toBe(3);
  });
});
